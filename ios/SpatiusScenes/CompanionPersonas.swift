...
///
/// The same set as the Web client's `companion-personas.ts`, word for word — the prompts
/// are what the character is, so a phone and a laptop talking to one backend have to be
/// talking to the same people.
struct Persona: Identifiable, Equatable {
    /// Also half of the memory key, so it must match the Web client's ids.
    let id: String
    /// Shown on the card.
    let name: String
...
enum CompanionPersonas {

    /// The id the custom entry files its memory under. A character written by hand has no
    /// stable name to file it under, so everything hand-written shares one per language.
    static let customId = "custom"

    static func all(for lang: Lang) -> [Persona] {
        lang == .zh ? zh : en
    }

    private static let zh: [Persona] = [
        Persona(
            id: "friend",
            name: "老朋友",
            blurb: "认识很久了，说话不用客气",
        ...
        Persona(
            ...
            icon: "📻",
            prompt: """
            你是用户所在行业的前辈，比对方早入行很多年，对方有事会来问问你的看法。
    ...
    private static let en: [Persona] = [
        Persona(
            ...
            icon: "☕",
            prompt: """
            You are a friend the user has known for years, and they have come round to talk.
        ...
        Persona(
            ...
            icon: "🌷",
            prompt: """
            You are a few years ahead of the user, and they come to you when something is
            on their mind.
        ...
        Persona(
            ...
            icon: "🍜",
            prompt: """
            You are the user's flatmate. You live together and talk about nothing in
            particular.
        ...
        Persona(
            ...
            icon: "📻",
            prompt: """
            You have worked in the user's field for many years, well before they started,
