...
/// question set rather than wrapping a Chinese stem in an English shell. Organized the
/// same way as the Web client's `i18n.ts`.
enum Lang: String {
    case zh
    case en
}
...
///
/// It starts out following the system and afterwards uses the user's last choice. On a
/// phone the system language is a clear signal, unlike the web where it has to be
/// guessed from navigator.language.
@MainActor
final class Localization: ObservableObject {
    static let shared = Localization()

    private static let key = "tutoring.lang"
    ...
    private init() {
        if let saved = UserDefaults.standard.string(forKey: Self.key),
           let parsed = Lang(rawValue: saved) {
            lang = parsed
        } else {
            let system = Locale.preferredLanguages.first ?? "en"
            lang = system.hasPrefix("zh") ? .zh : .en
        }
    }

    var t: Strings { lang == .zh ? Strings.zh : Strings.en }
}

/// One set of copy. Each language has an instance, with fields corresponding one to one.
struct Strings {

    // Config page
    let stepCredentials: String
    let stepAvatar: String
    let sectionBackendUrl: String
    let sectionCredentials: String
    let sectionAvatar: String
    let sectionTtsModel: String
    let sectionScene: String
    let fieldBackendUrl: String
    let fieldSampleRate: String
    let backendUrlHint: String
    let credentialsHint: String
    let credentialConfigured: String
    let credentialMissing: String
    let testConnection: String
    let connecting: String
    let backendOnline: String
    let connectFailedShort: (String) -> String
    let loadConfigFailed: (String) -> String
    let saveFailed: (String) -> String
    let next: String
    let back: String
    let start: String
    let saving: String
    let fillAllFirst: String
    let sampleRateNote: String
    let sceneTutoring: String
    let sceneLive: String
    let sceneService: String
    let sceneCompanion: String

    // Companion
    let enteringCompanion: String
    let companionListening: String
    let companionMicOff: String
    let companionMemory: String
    let companionMemoryEmpty: String
    let companionMemoryRaw: String
    let companionMemoryMeta: (Int, Int) -> String
    let companionForget: String
    let companionHello: String
    let companionHelloAgain: String
    let companionPick: String
    let companionPickHint: String
    let companionCustom: String
    let companionCustomBlurb: String
    let companionCustomPlaceholder: String
    let companionStart: String

    // Live room
    let enteringLive: String
    let liveBadge: String
    let viewerCount: (Int) -> String
    let greeting: String
    let you: String
    let chatPlaceholder: String
    let send: String
    let micIdle: String
    let micPending: String
    let micLive: String
    let micWelcome: String

    // Bank customer service
    let enteringService: String
    let serviceTitle: String
    let serviceOnline: String
    let serviceGreeting: String
    let serviceAsk: String
    let serviceSearch: String
    let serviceSearchEmpty: String
    let serviceLive: String
    let serviceLiveOn: String
    let serviceLiveListening: String
    let serviceLiveOpen: String
    let serviceCategories: String
    let serviceSpeaking: String
    let serviceLeave: String

    // Classroom
    let teacherComing: String
    let section: (Int) -> String
    let prevQuestion: String
    let nextQuestion: String
    let startFreeTalk: String
    let endFreeTalk: String
    let freeTalkListening: String
    let freeTalkLocked: String
    let teacher: String
    let me: String
    let micFailed: String
    let cameraOn: String
    let cameraOff: String
    let stagePreparing: String
    let stageLoadingAvatar: String
    let stageDownloading: (Int) -> String
    let stageConnecting: String
    let stageConnected: String
    let connectFailed: (String) -> String
    let offlineWaiting: String

    // Avatar lines
    let sayAllCorrect: String
    let sayCorrectThenChat: String
    let sayCorrect: String
    let sayFinishFirst: String
    let sayFreeTalkOpen: String
    let wrongReplies: [String]

    /// Assembles the full text for the avatar to read aloud. The phrasing differences
    /// (Chinese 「第一题」 vs English "Question 1") live in each language's own version,
    /// and callers just take the current one.
    let readAloud: (String, [String], Int) -> String

    let perfTitle: String
    let perfFps: String
    let perfPresentationFps: String
    let perfJank: String
    let perfFrameTime: String
    let perfIntervalP95: String
    let perfCpu: String
    let perfPlayback: String
    let perfFramesTotal: String
    let perfDropped: String
    let perfLost: String
    let perfStarved: String
    let perfWaiting: String
    let perfNote: String
    let perfDevice: String
    let perfThermal: String
    let perfBattery: String
    let perfThermalNominal: String
    let perfThermalFair: String
    let perfThermalSerious: String
    let perfThermalCritical: String
}
...
extension Strings {
    static let zh = Strings(
        ...
        connecting: "连接中…",
        backendOnline: "后台在线",
        connectFailedShort: { "连不上：\($0)" },
        loadConfigFailed: { "读取后台配置失败：\($0)" },
        saveFailed: { "保存失败：\($0)" },
        next: "下一步",
        back: "返回",
        start: "开始",
        ...
        companionMemoryMeta: { turns, size in "\(turns) 条对话 · \(size) 字" },
        ...
        serviceTitle: "数字客户经理",
        serviceOnline: "在线",
        // The Web version says 「在右边选择」 because the topics are a column beside the
        // avatar there. On a phone they are the sheet over the lower half of the screen.
        serviceGreeting: "您好，我是您的数字客户经理，很高兴为您服务。您可以在下方选择服务类型，也可以直接搜索，或者点实时问答开口问我。",
        serviceAsk: "猜您想问",
        ...
        // TTS reads 「第 1 题」 inconsistently, so numbers are mapped to Chinese
        // numerals; anything out of range falls back to Arabic digits. Options are
        // separated by 「。」 so they are not run together, and the 「选项」 prefix gives
        // TTS the context it needs — without it the letter A is read as 「啊」.
        readAloud: { stem, options, number in
            let table = ["一", "二", "三", "四", "五", "六", "七", "八", "九", "十"]
            let n = (1...10).contains(number) ? table[number - 1] : String(number)
            var out = "第\(n)题。\(stem)"
            for (index, option) in options.enumerated() {
                let letter = String(UnicodeScalar(65 + index)!)
                out += "。选项 \(letter)，\(option)"
            }
            return out
        },
        perfTitle: "性能",
        ...
        perfThermal: "发热状态",
        perfBattery: "电量",
        perfThermalNominal: "正常",
        perfThermalFair: "温热",
        perfThermalSerious: "偏热",
        perfThermalCritical: "过热"
    )
    ...
    static let en = Strings(
        ...
        connectFailedShort: { "Cannot reach it: \($0)" },
        loadConfigFailed: { "Could not read backend config: \($0)" },
        ...
        sceneCompanion: "Someone to talk to",
        ...
        companionMemoryMeta: { turns, size in "\(turns) turns · \(size) characters" },
        ...
        // The Web version says "on the right" because the topics are a column beside the
        // avatar there. On a phone they are the sheet over the lower half of the screen.
        serviceGreeting: "Hello, I am your digital account manager and I am glad to help. Pick a topic below, search for something specific, or tap Ask me anything and just ask.",
        ...
        readAloud: { stem, options, number in
            var out = "Question \(number). \(stem)"
            for (index, option) in options.enumerated() {
                let letter = String(UnicodeScalar(65 + index)!)
                out += ". Option \(letter), \(option)"
            }
            return out
        },
        ...
        perfThermal: "Thermal",
        perfBattery: "Battery",
        perfThermalNominal: "Nominal",
        perfThermalFair: "Fair",
        perfThermalSerious: "Serious",
        perfThermalCritical: "Critical"
    )
}

/// The Chinese/English toggle. Top-right, the same shape as on the Web client.
struct LangToggle: View {
    ...
    private func segment(_ title: String, lang: Lang) -> some View {
        let active = localization.lang == lang
        return Button {
